...
class MessageCenterMessagePage extends Component {
    ...
    componentDidMount() {
        ...
        $('#btnRemove').click(function (e) {
            ...
            $('#PairedSelectBox option:selected').each(function () {
                ...
                for (var i = smsArray.length - 1; i >= 0; i--) {
                    if (smsArray[i] === selectedData) {
                        smsArray.splice(i, 1);
        ...
        $('#btnRemoveAll').click(function (e) {
            ...
            $('#customerListTable').append($(selectedOpts).clone());
            var selectedData = "";
            smsArray.splice(0, smsArray.length);
    ...
    Submit() {
        ...
        if(smsArray.length>0 && this.state.message != undefined && this.state.message != ""){

        this.state.contactNo = smsArray.toString();
        this.setState({
         ...
         $.ajax({
                ...
                } else if (smsArray.length <= 0) {
                    Swal.fire({
                        position: 'center',
                        icon: 'warning',
                        // title: 'No Internet',
                        text: 'Select customers to send SMS',
                        showConfirmButton: false,
                        timer: 2000
                    })
                }
...
<h3 className="text-center">SMS Message</h3>
</div>
</div>
<div class="row">
...
<button type="button" class="btn btn-default" style={{ marginTop: "2%",
    backgroundColor: "rgb(5, 164, 181)", color: "white" }} onClick={() => this.CancelData()}
>Cancel</button>
</div>
</div>
</div>
...
<div class="row" >
    ...
    <div class="col-sm-2" style={{textAlign: "center" }}>
        <button id="btnAdd" style={{ width: "95px", marginBottom: "10px",backgroundColor:"#428bca",border:"1px #ccc",color:"#fff" ,padding:"5px 0px"}} value=">">Add</button><br />
        <button id="btnAddAll" style={{ width: "95px", marginBottom: "10px",backgroundColor:"#428bca",border:"1px #ccc", color:"#fff",padding:"5px 0px" }} value="<">Add All</button><br />
        <button id="btnRemoveAll" style={{ width: "95px", marginBottom: "10px",backgroundColor:"#428bca",border:"1px #ccc", color:"#fff",padding:"5px 0px" }} value="<">RemoveAll</button><br />
        <button id="btnRemove" style={{ width: "95px", marginBottom: "10px",backgroundColor:"#428bca",border:"1px #ccc", color:"#fff",padding:"5px 0px" }} value="<">Remove</button>
    </div>
...
<textarea
    ...
    rows="5" id="message"
></textarea>
<button type="button" class="btn btn-default" style={{ marginTop: "20px", marginBottom: "50px", marginRight: "10px", backgroundColor: "rgb(5, 164, 181)", color: "white" }}  onClick={() => this.Submit()}>
    Submit
</button>
</div>
);
