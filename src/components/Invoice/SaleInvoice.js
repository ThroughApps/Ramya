...
window.saleInvoiceComponent = this;
...
this.SaleInvoice_CurrencyChange=SaleInvoice_CurrencyChange.bind(this);
   ...
   //     alert("SAVE INVOICE WITH ENQUIRY CALLED");
     ...
     //   alert("SAVE INVOICE CALLED");
      ...
      && (self.state.productQtyMinusModal==false || self.state.productQtyMinusModal==undefined)){
        ...
        var toastSpecification={
          message:"! Cart is empty ",
          position:"top-right"
        };

        ToastWarningIcons(toastSpecification);

       // <ToastWarningIcons message="! Cart is empty "  position="top-right"/> ;
        
        /*toast.error(<div><span style={{display: "flex" }}><ToastWarningIcons />! Cart is empty </span></div>, {
            position: "top-right",
            autoClose: 5000,
...
SetCustomerData(e){
  ...
  this.setState({
    ...
    companyName:this.state.companyName,
  	...
  	//CHECK IF EMAILID EXIST -- SET ALERT IF EMAILID DOESNOT EXIST
...
*/
CheckInventory(value,productRate,productCGST_Percentage,
...
//alert("this.state.productRate_Each :"+this.state.productRate_Each);	
...
if(name=="productDiscountAmount"){
    ...
    var percentFormat = formatter.format( (value /100) * Number(this.state.productTotal));
...
Check_Discount_Exceeds_Rate(){
 ...
 // var productFinalAmount=this.state.productFinalAmount - this.state.productDiscountAmount;
  ...
  console.log("this.state.productFinalAmount :",this.state.productFinalAmount );
  console.log("this.state.productSubtotalAmount  :",this.state.productSubtotalAmount );
...
var cartProductId=self.state.cartProductId;
 ...
 var cartProductRate=self.state.cart_ItemRate;	
          ...
          self.SetProduct_ServiceDetails(cartProductId,self.state.productQuantity,cartProductRate,	
            ...
            cartProductDiscount_Percentage,cartProductDiscountAmount);//this.UpdateInventory(cartProductId,this,this.state.productQuantity,"MinusQuantity");
//this.UpdateProductDropDown();
self.DeleteProduct_ServiceFunc(rowIndexValue,self.state.productQuantity,	
  cartProductId);
...
ProductQtyAddcloseModal(buttonData){
  ...
  var inventoryProductDetails=_.findWhere(this.state.productInventoryArrayList,{productId:self.state.cartProductId});
    ...
    //CHECK WITH INVENTORY QUANTITY
    if(Number(inventoryProductDetails.productQty) >=  Number(newQty)){
...
AddEnquiryFunc(productId,productQty,inventoryProductDetails){
 ...
 if(checkEnquiryDataExist==undefined){
    var newEnquiryProductData={
      ProductId:productId,
      Product:inventoryProductDetails.productName,
  ...
  */
  SaveInvoiceFunc(){
      ...
      if(this.state.cartData.length>0){
        ...
        if(this.state.invoiceAmt==0.00 || this.state.invoiceAmt=="" || this.state.invoiceAmt==undefined ){
          toast.error("! Invoice amount cannot be empty or zero ", {
            position: "top-right",
            autoClose: 5000,
            hideProgressBar: false,
            closeOnClick: true,
            ...
            marginTop: "60px",
        ...
        }else if(this.state.paymentStatus!="UnPaid" && this.state.paymentMode==""){
              ...
              $.ajax({
                ...
                url: " http://15.206.129.105:8080/ThroughBooksCOAPI/saleInvoice/AddSaleInvoiceDetails",
      ...
      <div className="col-lg-8" style={{ backgroundColor: ""}}>
        ...
        <div class="row" style={{marginTop:'15px'  }}>
          ...
          <div class="col-xs-12 col-sm-2 col-lg-2 ">
            ...
            <label>
              ...
              <button onClick={() => this.ProductClearFunc()} type="button" style={{ marginTop: "25px" }} class="btn btn-default pull-right ">Clear</button> <span></span>
            </label>
          </div>
        ...
        <div class="">
          <form class="form-horizontal form-bordered">
            ...
            <div class="table-responsive">
              <table class="table" style={{  marginBottom: "0px",marginTop:'-10px'  }}>
                <thead>
                  <tr>
   ...
   <div className="col-lg-4">
    <a href="#" className="user_p_menu"
      ...
      style={{ color: "blue", fontWeight: "800", textDecorationLine: "underline",
       marginLeft:'240px',fontSize:'18px' }} >Invoice Enquiry</span>
      </span>
      </a>
  </div>
...
export const SaleInvoice_CurrencyChange = (InvoicePageStates) => {
...
console.log("InvoicePageStates :",InvoicePageStates);
 ...
 window.saleInvoiceComponent .CurrencyFormat();

}
 


export const PopulateFields = (currentState,AjaxData,
  AjaxData_ProductList,calledFrom) => {
...
*/
    function  CloseInfo(){
      // alert("closeinfo called ");
       Notification.close();
       window.saleInvoiceComponent.NotificationClose();
      }