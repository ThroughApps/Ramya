...
import { GetEmployeeSite,GetCurrentSite, GetSiteDetails, 
  GetCurrencies,SetCurrentPage, OffsetValueCalcFunc, GetDynamicFieldsName   } from '../ConstSiteFunction';
...
import { InvoiceCartIcons, InformationIcon  } from '../ServiceRegistration/IconComponents';
...
class SaleInvoiceUpdate extends Component {


constructor(data) {
super(data)

window.saleInvoiceUpdateComponent = this;
...
product_CGST_Percentage:'',
product_SGST_Percentage:'',
productFinalAmount:'',

productQtyAddModal:false,  
productQtyMinusModal:false,
discountReadOnly:true,
invoiceAmtPaidReadOnly:true,
...
this.SaleInvoiceUpdate_CurrencyChange=SaleInvoiceUpdate_CurrencyChange.bind(this);
...
componentDidMount() {
  ...
  //console.log("COMPO DID MOUNT :",this.props);
  ...
  SetCurrentPage("SaleInvoiceUpdate");
...
console.log(this.state.tax1LabelName);
console.log(this.state.tax2LabelName);
...
invoiceNo:this.props.id,
tableName:"SaleInvoiceTable",

}),
url: " http://15.206.129.105:8080/ThroughBooksCOAPI/saleInvoice/SelectBasicInvoiceDetailsUpdate",
...
//SET CUSTOMER INFO
var custInfo=_.findWhere(AjaxData.customerList,{customerId:AjaxData.invoiceProductList[0].customerId});
var evalue={label:custInfo.customerName+ " "+custInfo.cust_ContactNo,value:AjaxData.invoiceProductList[0].customerId  };
self.SetCustomerData(evalue);
...
ChangeCustomerFunc(){
  ...
  PopulateFields(this,AjaxData,AjaxData_ProductList,"FromCustomerChange");
  this.CancelFunc();
...
handleProductDetails = (e) => {
  ...
  if(productDetails.productType=="product"){
    $('#productQuantity').attr('readonly', false);
     ...
     this.CheckInventory(value,productDetails.productRate,productDetails.product_CGST_Percentage,	
...
AVAILABLE IN THE LOCAL INVENTORY
*/
  CheckInventory(value,productRate,productCGST_Percentage,productSGST_Percentage,
  ...
  var inventoryProductDetails=_.findWhere(this.state.productInventoryArrayList,{productId:value});
    ...
    if(inventoryProductDetails.productQty >=  this.state.productQuantity){
      ...
      //this.SetProduct_ServiceDetails(value,this.state.productQuantity);
      this.SetProduct_ServiceDetails(value,this.state.productQuantity,productRate,productCGST_Percentage,	
...
this.state.productRate_Each=Truncate_2DecimalPlaces(productRate);
  this.state.product_CGST_Percentage=productCGST_Percentage;
  this.state.product_SGST_Percentage=productSGST_Percentage;
...
Check_Discount_Exceeds_Rate(){
 ...
 // var productFinalAmount=this.state.productFinalAmount - this.state.productDiscountAmount;
  ...
  if(Number(this.state.productDiscountAmount) > Number(this.state.productFinalAmount) ){
        this.state.productDiscountAmount=0;
        this.state.productDiscountPercentage=0;
  ...
  //UPDATE PRODUCT INVENTORY
    ...
    if(value!="" && value!= undefined){
      ...
      if(this.state.productQuantity!="" && this.state.productQuantity!=0 && this.state.productQuantity!=undefined){
        ...
        if( (this.state.productRate_Each!="" && this.state.productRate_Each!=undefined && this.state.productRate_Each!=0.00) &&
        (this.state.productFinalAmount!="" && this.state.productFinalAmount!=undefined && this.state.productFinalAmount!=0.00) ){
...
if(productDetails.productType=="service"){
  ...
  var newCartData={
    ...
    EntryType:'New',
    ExistedQty:productQty,
    NewQty:productQty,
  }

  this.state.cartData.unshift(newCartData);
  this.state.oldCartData=[];
  this.state.oldCartData=[...this.state.cartData];

  this.setState({
    cartdata:this.state.cartdata,
    oldCartData:this.state.oldCartData,
  })
...
this.PopulateCartData(cartData,0);
 ...
 // console.log("this.state.cartData :",this.state.cartData);
  //CLEAR THE PRODUCT FIELDS
    ...
    toast.error("Rate & FinalAmount of "+this.state.productName+" cannot be empty or zero !", {
      position: "top-right",
      autoClose: 5000,
      hideProgressBar: false,
      closeOnClick: true,
    ...
    localStorage.setItem('CartDataInfo', CryptoJS.AES.encrypt(this.state.cartDataInfo, "shinchanbaby"));
        ...
        $.each(cartData, function (i, item) {
          ...
          if(! _.isEmpty(cartData[i]) && cartData[i] !=undefined){
              ...
              'Qty': item.Qty,
              'Total':item.Total,
              'DiscountPercentage': Truncate_2DecimalPlaces(item.DiscountPercentage),
        ...
        console.log(" BEFORE IF CALLED self.state.deletedCartData :",       self.state.deletedCartData)

            if(Number(self.state.invoiceAmt_Paid) > Number(invoiceAmt) ){
             // alert("IF CALLED");

              console.log(" IF CALLED self.state.deletedCartData :",       self.state.deletedCartData)
              self.state.cartData=[];
              var deletedCartData_Pop=[... self.state.deletedCartData];

              deletedCartData_Pop.pop();

              self.setState({
                cartData:self.state.cartData,
                deletedCartData:deletedCartData_Pop,
              })

              console.log(" AFTER IF CALLED POP self.state.deletedCartData :",       self.state.deletedCartData)
         

              var count=0;
              var arrayCount=0;

              console.log(" populate cart data if block self.state.oldCartData :",self.state.oldCartData);

            if(self.state.oldCartData.length>0){
              this.state.cartDataInfo='NotEmpty';
              localStorage.setItem('CartDataInfo', CryptoJS.AES.encrypt(this.state.cartDataInfo, "shinchanbaby"));
          
              $.each(self.state.oldCartData, function (i, item) {

                
          if(! _.isEmpty(self.state.oldCartData[i]) && self.state.oldCartData[i] !=undefined){
              ...
              self.setState({
                ...
                toast.error("! Invoice Amount is less than amount paid already", {
                    position: "top-right",
                    autoClose: 5000,
                    hideProgressBar: false,
                    closeOnClick: true,
             ...
             // alert("ELSE CALLED");
              ...
              this.statebalanceAmt="";
                ...
                self.state.cartData=cartDataArray;
  ...
  if (typeof rowInfo !== "undefined") {
    return {
      onClick: (e, handleOriginal) => {
        ...
        self.state.cart_ItemEntryType=rowInfo.original["EntryType"];
        self.state.cart_ItemExistedQty=rowInfo.original["ExistedQty"];
        self.state.cart_ItemNewQty=rowInfo.original["NewQty"];
        ...
        self.setState({
          ...
          cart_ItemEntryType:self.state.cart_ItemEntryType,
          cart_ItemExistedQty:self.state.cart_ItemExistedQty,
          cart_ItemNewQty:self.state.cart_ItemNewQty,
...
this.state.productQuantity=newQty;
this.state.productId=cartProductId;
if(newQty==""){
  this.state.productQuantity=1
}
...
ProductQtyAddcloseModal(buttonData){
  ...
  var updatedQty=Number(self.state.cart_ItemNewQty)+Number(this.state.newQtyAdded);

  var inventoryProductDetails=_.findWhere(this.state.productInventoryArrayList,{productId:self.state.cartProductId});

    //CHECK WITH INVENTORY QUANTITY
    if(Number(inventoryProductDetails.productQty) >=  Number(updatedQty)){
      this.ReplaceCart_With_Updated_Qty(newQty,updatedQty);
      this.UpdateInventory(self.state.cartProductId,this,this.state.newQtyAdded,"AddQuantity");
      this.UpdateProductDropDown();
    }else{

      this.AddEnquiryFunc(this.state.productId,this.state.newQtyAdded,inventoryProductDetails);
...
ProductQtyMinuscloseModal(buttonData){ 
  ...
  if(buttonData=="Save"){
    ...
    var updatedQty=Number(self.state.cart_ItemNewQty)-Number(this.state.newQtyMinus);

   // var proceedFurtherData=true;
    if(self.state.cart_ItemEntryType=="Old" && (Number(self.state.cart_ItemNewQty)==0)){
     
      if(self.state.cartItem_ProductType=="product"){
      var deleteData={
        ProductId:self.state.cartProductId,
        Item: self.state.cartItem,
        ProductType:self.state.cartItem_ProductType,
        Qty: this.state.newQtyMinus,
     
       }

       this.state.newQtyMinus=0;

        this.state.deletedCartData.push(deleteData);
  
        this.setState({
          deletedCartData:this.state.deletedCartData,
        })
      }

      console.log(" ProductQtyMinuscloseModal self.state.deletedCartData :",       self.state.deletedCartData)
...
//if(proceedFurtherData==true){
 ...
 /* }else{
    toast.error("! Invoice amount paid exceeds the invoice amount", {
      position: "top-right",
      autoClose: 5000,
      hideProgressBar: false,
      closeOnClick: true,
...
DeleteProduct_ServiceFunc(rowIndexValue,newQty,cartProductId){
 ...
 // if(proceedFurtherData==true){
  ...
  if(this.state.cart_ItemEntryType=="Old" && self.state.cartItem_ProductType=="product"){
    var deletedCartData= this.state.cartData[rowIndexValue];
    this.state.deletedCartData.push(deletedCartData);
  }

  console.log("***** DeleteProduct_ServiceFunc *******this.state.deletedCartData :",this.state.deletedCartData);
...
*/
  ...
  //  console.log("ProductQtyMinusFunc :",value);
    ...
    */
  ReplaceCart_With_Updated_Qty(newQty,updatedQty){
   ...
   //   alert("ReplaceCart_With_Updated_Qty called "+newQty);
    ...
    //  var totalQty=Number(newQty)+Number(updatedQty);
  ...
  this.state.oldCartData=[];
  this.state.oldCartData=[...self.state.cartData];
      ...
      self.setState({
        cartData:self.state.cartData,
        oldCartData:self.state.oldCartData,
       })
      ...
      var newCartData={
        ...
        EntryType:self.state.cart_ItemEntryType,
        ExistedQty:self.state.cart_ItemExistedQty,
        NewQty:updatedQty,
       }
...
DeleteCartProductFunc(){
  ...
  if(  self.state.cartProductId!==""){
   ...
   // var proceedFurtherData=true;
     ...
     if(this.state.cart_ItemEntryType=="Old"){
      //CHECK FOR INVOICE AMT CHANGES
   //   proceedFurtherData=this.Check_InvoicePaidAmt_Exceeds_InvoiceAmt(this.state.cartData,rowIndex,'SingleProduct',0);
  ...
  //  if(proceedFurtherData==true){
      ...
      if(this.state.cart_ItemEntryType=="Old" && self.state.cartItem_ProductType=="product"){
      var deleteData=this.state.cartData[rowIndex];
      this.state.deletedCartData.push(deleteData);
      ...
      console.log("***** DeleteCartProductFunc *******this.state.deletedCartData :",this.state.deletedCartData);
  ...
  if(  self.state.cartData.length > 0){
   ...
   // console.log("DeleteAllProductInCartFunc :",self.state.cartData);
      ...
      if(Number(self.state.invoiceAmt_Paid)==0){
              ...
              var cartDeletedOldData=_.where(self.state.cartData,{EntryType:'Old',ProductType:'product'});

              this.state.deletedCartData.push(cartDeletedOldData);
              console.log("***** DeleteAllProductInCartFunc *******this.state.deletedCartData :",this.state.deletedCartData);

              self.state.cartData=[];
              self.setState({
                cartData:self.state.cartData,
                deletedCartData:self.state.deletedCartData,
              })
              ...
              toast.error("! Cart cannot be deleted since invoice amount is paid ", {
                position: "top-right",
                autoClose: 5000,
                hideProgressBar: false,
                closeOnClick: true,
...
//FUCNTION USED TO ADD THE OUT OF STOCK PRODUCTS TO ENQUIRY MODULE
      ...
      if(this.state.cartData.length>0){
           ...
           /* if(this.state.paymentStatus!="UnPaid" && this.state.paymentMode==""){
              ...
              */
              ...
              console.log("UPDATE SALE INVOICE AJAX DATA :",JSON.stringify({
                companyId: this.state.companyId,
              ...
              //STAFF INFO
                ...
                deletedCartData:JSON.stringify(this.state.deletedCartData),
              ...
              $.ajax({
                ...
                //STAFF INFO
                  ...
                  totalIGSTAmt:this.state.totalIGSTAmt,
                  whatsAppOption:true,
                  oldPaymentStatus:this.state.paymentStatus,
                ...
                url: " http://15.206.129.105:8080/ThroughBooksCOAPI/saleInvoice/UpdateSaleInvoiceDetails",
                ...
                success: function (data, textStatus, jqXHR) {
                  ...
                  AjaxData=data;
                    if(data.invoiceResponse=="Success"){
                      ...
                      PopulateFields(self,AjaxData,AjaxData_ProductList,"FromUpdateAjax");
...
/*********************************************************************************
  ...
  ************************************************************************************/
   ...
   InfoDetails(){	
    var self=this;	
    Notification['info']({	
      title: 'Shortcut keys provided for sale invoice edit',	
 ...
 BackbtnFunc() {
     if(this.state.cartData.length>0){
        ...
        Swal.fire({
            ...
            icon: 'warning',
         //   title: 'Product is not sufficient',
            text:'Sale Invoice Edit is in progress, moving back might delete the invoice data',
            showConfirmButton: true,
            showCancelButton: true,
        ...
        <div class="row" style={{ backgroundColor: "" }}>
          <div class="col-xs-12 col-sm-4 col-lg-4 ">
            ...
            <SelectSearch options={this.state.customerOptions} value={this.state.selectedCustomerName} id="customerName"
              onChange={(e) => this.handleCustomerNameDetails(e)} name="customerName" readonly={true} placeholder="Select Customer" />
          ...
          {/* <a href="#myModal" data-toggle="modal" data-target="#myModal" >
              <span
                style={{
                  color: "blue",
                }} onClick={() => this.AddCustomerQuickLinkFunc()}  > Add Customer</span>
            </a> */}
         </div>
        ...
        <div class="row" style={{ backgroundColor: "" }}>
          <div class="col-xs-12 col-sm-4 col-lg-4 ">
            ...
            <input readOnly type="text" class="form-control" value={this.state.orderNumber}
             onChange={this.handleUserInput} name="orderNumber" id="orderNumber" readOnly/>
            ...
            <div class="table-responsive">
              <table class="table" style={{ marginBottom: "0px" }}>
                <thead>
                  ...
                  <th >SubTotal</th>
                    <th>{this.state.tax1LabelName}(%)</th>
                    <th>{this.state.tax2LabelName}(%)</th>
                    <th>Final Amount</th>
                  </tr>
                </thead>
                <tbody>
            ...
            <div class="">
              <div class="table-responsive">
                ...
                <table class="table" style={{ borderTop: "none", marginTop: "20px" }} >
                  <tbody>
                    <tr><th style={{ width: "50%", borderTop: "none" }}>TotalWithout Tax</th>
                    ...
                    <tr><th style={{ width: "50%", borderTop: "none" }}>Total Tax</th>
                    ...
                    <tr><th style={{ width: "50%", borderTop: "none" }}>Discount</th>
                      ...
                      <span id="currencysymbolSpan" style={{fontSize: "19px",marginRight: "10px"}}>{this.state.currencySymbol}</span>
                        <input name="discountAmt" type="text" id="discountAmt"
                          value={this.state.discountAmt} readOnly={this.state.discountReadOnly}
                    ...
                    <tr><th style={{ width: "50%", borderTop: "none" }}>Amount Paid</th>
                      ...
                      <span id="currencysymbolSpan" style={{fontSize: "19px",marginRight: "10px"}}>{this.state.currencySymbol}</span>
                        <input name="invoiceAmt_Paid" type="text" id="invoiceAmt_Paid"
                          value={this.state.invoiceAmt_Paid} readOnly={this.state.invoiceAmtPaidReadOnly}
                          onChange={this.handleUser_Summary_Discount_AmountPaid_CalcFunc}
                          onKeyPress={this.handleUser_Summary_Discount_AmountPaid_KeyPress_Func}
              ...
              onChange={(e) => this.handleAdvisorDetails(e)} name="selectedAdvisor" placeholder="Select Advisor" />
                    ...
                    <tr>
                      ...
                      <td style={{ borderTop: "none", marginRight: "-24px" }}>
                        <button onClick={() => this.InvoiceCancelFunc()} type="button" class="btn btn-default ">Cancel</button>
                      </td>
...
export default SaleInvoiceUpdate;



export const SaleInvoiceUpdate_CurrencyChange = (InvoicePageStates) => {
...
console.log("InvoicePageStates :",InvoicePageStates);
 ...
 window.saleInvoiceUpdateComponent .CurrencyFormat();
...
var productListData=AjaxData.invoiceProductList;
...
if(calledFrom=="FromSelectUpdateDetails"){
...
$.each(productListData, function (i, item) {
    ...
    console.log("POPULATE CART DATA ITEM :",item);
        ...
        'DiscountAmt': item.productDiscountAmount,
        'SubTotal': item.productSubTotal,
        'CGST_Percentage': item.product_CGST_Percentage,
        'SGST_Percentage': item.product_SGST_Percentage,
        'CGSTAmt':item.product_CGST_Amt,
        'SGSTAmt':item.product_SGST_Amt,
        'FinalAmt': item.productFinalAmt,
        'EntryType':'Old',
        'ExistedQty':item.productQty,
        'NewQty':0
        }

        totalItemQty=Number(totalItemQty)+item.productQty;
...
self.state.totalWithoutGST=Truncate_2DecimalPlaces(Number(productListData[0].invoiceAmt)-Number(productListData[0].totalGst));
self.state.totalGst=productListData[0].totalGst;
self.state.invoiceAmt=productListData[0].invoiceAmt;
self.state.discountAmt=productListData[0].discountAmt;
self.state.invoiceAmt_Paid=productListData[0].invoiceAmt_Paid;
self.state.balanceAmt=productListData[0].balanceAmt;
self.state.paymentStatus=productListData[0].paymentStatus;
self.state.paymentMode=productListData[0].paymentMode;
self.state.selectedPaymentMode={label:productListData[0].paymentMode,value:productListData[0].paymentMode};
self.state.invoiceAdvisor=productListData[0].invoiceAdvisor;
self.state.invoiceAdvisorId=productListData[0].invoiceAdvisorId;
self.state.selectedAdvisor={label:productListData[0].invoiceAdvisor,value:productListData[0].invoiceAdvisorId};
self.state.totalItemQty=totalItemQty;
self.state.oldPaymentStatus=productListData[0].paymentStatus;

var amountInWords=convert_to_words(self.state.invoiceAmt);
self.state.amountInWords=amountInWords;

//alert("productListData[0].paymentStatus :"+productListData[0].paymentStatus);
if(productListData[0].paymentStatus=="UnPaid"){
 // alert("productListData[0].paymentStatus :"+productListData[0].paymentStatus);

  self.state.discountReadOnly=false;
  self.state.invoiceAmtPaidReadOnly=false;
}
...
self.setState({
  ...
  */
  ...
  oldPaymentStatus:self.state.oldPaymentStatus,

  discountReadOnly:self.state.discountReadOnly,
  invoiceAmtPaidReadOnly:self.state.invoiceAmtPaidReadOnly,
  ...
  self.setState({
    ...
    window.invoiceMenuPageComponents.InvoicePagesCartDataInfo(self.state.cartData);
      }
  ...
  */
  ...
  /*self.state.cartData=[];
    ...
    */
    ...
    window.saleInvoiceComponent.PopulateCartData(cartData,0);
...
/*
  ...
  */
     ...
     /*
    USED TO CLOSE THE NOTIFICATION MESSAGE OF THE PAGE
    */
    function  CloseInfo(){	
      // alert("closeinfo called ");	
       Notification.close();	
       window.saleInvoiceUpdateComponent.NotificationClose();	
      }