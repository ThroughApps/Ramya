...
class ImportQRCode extends Component {
  ...
  handleUpload() {
    ...
    $.ajax({
      ...
      method: "POST",
       ...
       url: " http://15.206.129.105:8080/ThroughBooksCOAPI/Login/qrCodeUpload",
...
//  xhr.open("POST", " http://15.206.129.105:8080/ThroughBooksCOAPI/Login/UploadFile");
//     // xhr.open(
//     //   "POST",
...
//       });
    ...
    console.warn("ModifiedName",this.state.companyId+form.name);
    var qrCodeFileName=this.state.companyId+form.name;

    var formData = new FormData();
    formData.append('newName',qrCodeFileName);
    formData.append("formData", form,qrCodeFileName);
 ...
 xhr.open("POST", " http://15.206.129.105:8080/ThroughBooksCOAPI/Login/UploadQRFile");
    // xhr.open(
    //   "POST",
    //   "http://13.127.217.180:8080/ResumeSite/ResumeSite/UploadFile"
    // );
   ...
   localStorage.setItem('CompanyQRCode', CryptoJS.AES.encrypt(qrCodeFileName.toString(), "shinchanbaby"));
    ...
    return (
      ...
      <div className="">
        ...
        <header className="App-header">
          <h3 style={{fontSize:"30px"}} className="App-title">Upload your QR Code For Payment:</h3>
        </header>
