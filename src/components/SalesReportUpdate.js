...
class SalesReportUpdate1 extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      oldBalanceAmt: this.props.balanceAmt,  //OLD BALACE AMOUNT - AMOUNTIN OENDING TO BE PAID
      oldSubtotal1:this.props.subtotal1, //OLD SUBTOTAL -TOAL OLD INVOICE AMOUNT
      oldAdvance:this.props.advance, //PAID AMOUNT FOR THE INVOICE BEFORE EDIT ,
  ...
  componentDidMount() {
    SetCurrentPage("SalesReportUpdate");

    console.log("THIS.PROPS.DATA :",this.props);

    console.log("alert :",this.state.oldSubtotal1);
...
Swal.fire({
  ...
  SelectCustomer() {
    ...
    $.ajax({
      ...
      url: " http://15.206.129.105:8080/ThroughBooksCOAPI/saleorder/selectProductDetails",
        ...
        $.each(data, function (i, item) {
          ...
          if (item.product != null) {
            ...
            tab += '<tr><td>' + self.state.productName + '</td>'
              ...
              +'<td>' + self.state.prefinalAmount1 + '</td><td>' + self.state.cgsta + '</td>'
...
var advanceRedeemamount=(Number(this.state.advance));
  ...
  SaleInvoiceFunc() {
    ...
    $("#tablecontents tr").each(function () {
      ...
      var productType = currentRow.find("td:eq(14)").text();
      ...
      if ((this.state.invoiceData.trim().length > 1)) {
        if ((this.state.invoiceDate.trim().length > 1) && (this.state.dueDate.trim().length > 1)) {
          ...
          // SET PAYMENT STATUS
                  ...
                  if(this.state.oldBalanceAmt==0 && this.state.balance_amount==0 ){
                      this.state.payment_status="Paid";

                    }else if(this.state.oldBalanceAmt==0){
                      this.state.payment_status="PartiallyPaid";

                  }else if(this.state.oldBalanceAmt==this.state.oldSubtotal1){
                    this.state.payment_status="UnPaid";

                  }
                
                  //FOR SAVE INVOICE WITH PROPMT

                  if(this.state.advance > this.state.subtotal1){
                        Swal.fire({
                          icon:"warning",
                          title: 'Invoice Cannot be Saved,Since Paid Amount is Greater than Invoice Amount !!',
                          showConfirmButton:false,
  ...
  EditAjaxCall(){
   ...
   url: " http://15.206.129.105:8080/ThroughBooksCOAPI/saleorder/updatesaleorder",
      contentType: "application/json",
      dataType: 'json',
      async: false,
      success: function (data, textStatus, jqXHR) {
  ...
  ResponseCalculation(data) {
    ...
    $.each(data.responseListdata, function (i, item) {
      ...
      tab += '<tr style={{backgroundColor: "white",fontColor:"black",fontWeight:"bold"}} id="tabletextcol" >'
        ...
        +'<td>' + item.prefinalAmount + '</td><td>' + item.cgst + '</td><td>' + item.sgst + '</td>'
    ...
    return (
     ...
     <div class="">
          <div class="" style={{ backgroundColor: "white" }}>
           <h3 style={{ textAlign: "center", marginTop: "8px" }}>Sale Invoice</h3>
           <hr></hr>
          </div>
