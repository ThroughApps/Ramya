...
import './Topnavbar/GenericDashboardCSS.css';
import './Topnavbar/TopNavbarCss.css';
...
import LanguagePage from '../SaleInvoice/LanguagePage';
...
export default class NewMenuBar extends Component {
    ...
    OpenModal(status) {
      ...
      if(PreviousVistedPage=="SaleOrder"){

      
      //  alert("WARNING SALE ORDER !!!");

      this.CheckCartCount("Sale Order",status);

      }else if(PreviousVistedPage=="EstimateInvoice"){
       // alert("WARNING ESTIMATE INVOICE !!!");

       this.CheckCartCount("Estimate Invoice",status);
    ...
    CheckCartCount(moduleName,status){

        var cartRowCount = CryptoJS.AES.decrypt(localStorage.getItem('CartRowCount'),"shinchanbaby").toString(CryptoJS.enc.Utf8)
  
        alert("cartRowCount :"+cartRowCount);

        if(cartRowCount=="Empty"){
            this.DisplayModal(status);
                            ...
                            <SidebarContent>
                                <Menu iconShape="round">
                                    ...
                                    >
                                        ...
                                        <MenuItem onClick={() => this.SaleInvoiceFunc()}>Sale Invoice</MenuItem>
